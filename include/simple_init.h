...
#include <stdlib.h>     // malloc(), free(), atoi(), exit(), srand(), rand()
#include <sys/time.h>   // gettimeofday()
#include <time.h>       // time() <- para semilla aleatoria
#include <math.h>       // pow()

/**
 * Valida la cantidad de parámetros y modifica variables globales de N, K, delegando la tarea al 
 * master de informar en caso de haber error.
 * 
 ...
 * @return int          el cual indica el valor de retorno: 0 si se realizó con éxito u otro valor 
 *                      distinto en el caso contrario
*/
...
/**
 * Inicializa los vectores globales con valores aleatorios iguales en ambos vectores, agregando K 
 * valores diferentes en posiciones aleatorias del vector 2. Considera que ambos vectores son del 
 * mismo tamaño.
 * 
 * @param vec1          vector 1
 * @param vec2          vector 2
